...
def plot_gem5_microarch():
    """Generates a grouped bar chart comparing cycles and cache misses from gem5 simulation."""
    csv_path = os.path.join(RESULT_DIR, "gem5/gem5_summary.csv")
    if not os.path.exists(csv_path):
        print(f"Warning: gem5 summary CSV {csv_path} not found. Skipping plot.")
        return
        
    df = pd.read_csv(csv_path)
    
    # Filter for default cache configuration (L1D=64kB, L2=512kB)
    df_def = df[(df["l1d_size"] == "64kB") & (df["l2_size"] == "512kB")]
    if df_def.empty:
        print("Warning: No matching default gem5 cache sizes in summary. Skipping plot.")
        return
        
    # Order by queue type
    df_def = df_def.set_index("queue_type").reindex(["centralized", "distributed", "lock_free"]).reset_index()
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(7.0, 3.2))
    
    # 1. Cycles plot
    bars1 = ax1.bar(df_def["queue_type"], df_def["cpu_cycles"] / 1e6, 
                    color=["#d7191c", "#fdae61", "#2c7bb6"], edgecolor='black', width=0.5)
    ax1.set_ylabel("CPU Cycles (Millions)")
    ax1.set_title("CPU Overhead (Lower is Better)")
    ax1.set_xticklabels(["Centralized", "Distributed", "Lock-Free"], fontsize=8)
    
    # Add values on top of bars
    for bar in bars1:
        yval = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2.0, yval + 0.5, f"{yval:.2f}M", 
                 ha='center', va='bottom', fontsize=8, weight='bold')

    # 2. Cache Misses plot
    x = np.arange(len(df_def["queue_type"]))
    width = 0.35
    
    ax2.bar(x - width/2, df_def["dcache_misses"], width, label="L1 D-Cache Misses", 
            color="#abd9e9", edgecolor='black')
    ax2.bar(x + width/2, df_def["l2cache_misses"], width, label="L2 Cache Misses", 
            color="#2c7bb6", edgecolor='black')
    
    ax2.set_ylabel("Cache Miss Count")
    ax2.set_title("Cache Performance")
    ax2.set_xticks(x)
    ax2.set_xticklabels(["Centralized", "Distributed", "Lock-Free"], fontsize=8)
    ax2.legend()
    
    plt.suptitle("Microarchitectural Queue Design Comparison (gem5)", fontsize=10, weight='bold')
    plt.tight_layout()
