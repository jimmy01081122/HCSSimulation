...
def make_gate_hook(layer_idx):
    def hook(module, input_states, output_logits):
        ...
        with torch.no_grad():
            scores = torch.softmax(logits.float(), dim=-1) # convert to float for stability
            k = 4
            topk_scores, topk_indices = torch.topk(scores, k=k, dim=-1)
            num_tokens = logits.size(0)
            
            # Record for the last token in sequence
            for r in range(k):
...
def main():
    ...
    print("=========================================================")
    print("  Qwen1.5-MoE-A2.7B GPTQ-Int4 GSM8K Profiling Pipeline   ")
    print("=========================================================")
    ...
    print(f"Loading GPTQ model: {model_id} on CPU...")
    t0 = time.time()
    
    # Load model on CPU using AutoModelForCausalLM. GPTQ models support CPU execution in modern transformers/optimum.
