...
- **RTL-Python Alignment:** Control-path modules (criticality-aware arbiter, prefetch drop controller, prefetch confidence filter, and deadline tracker) were simulated in SystemVerilog and verified. RTL matches the Python golden model at request completion timestamp level, while internal cycle-level arbitration states differ due to hardware pipeline timing.
...
* **Stall Alignment:** Stalls for all policies are measured consistently in clock cycles (Mcyc).
...
### A. Request and Cycle-Level Metrics
...
1. **Aggressive Standalone Cache Prefetching:** Standalone prefetch caches can achieve high hit rates (54.40%), but the resulting traffic expansion of 14.56x (4.6 TB) degrades latency by 0.09x due to PCIe bus congestion. Hardware prefetching is only viable when paired with a co-scheduled arbiter.
...
2. *Proposed scheduling policy outperforms the Oracle prefetcher:* Disproven (Oracle is the absolute mathematical limit under bus capacity bounds).
3. *MoE routing popularity is highly skewed, making static caching highly effective:* Disproven (global popularity skew is very low, Gini = 0.0692).
